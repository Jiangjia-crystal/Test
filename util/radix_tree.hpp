...
public:
    ...
    iterator altree_find(const K &key);
    iterator begin();
    iterator end();
...
private:
    ...
    radix_tree_node<K, T, Compare>* altree_find_node(const K &key, radix_tree_node<K, T, Compare> *node, int depth, bool compare, radix_tree_node<K, T, Compare> *prev_node);
...
template <typename K, typename T, typename Compare>
typename radix_tree<K, T, Compare>::iterator radix_tree<K, T, Compare>::altree_find(const K &key)
{
    if (m_root == NULL)
        return iterator(NULL);
    
    radix_tree_node<K, T, Compare> *node = altree_find_node(key, m_root, 0, true, nullptr);
...
radix_tree_node<K, T, Compare>* radix_tree<K, T, Compare>::find_node(const K &key, radix_tree_node<K, T, Compare> *node, int depth)
{
    ...
    typename radix_tree_node<K, T, Compare>::it_child it;
    int len_key = radix_length(key) - depth;  // the length of the remaining key to be compared.

    for (it = node->m_children.begin(); it != node->m_children.end(); ++it) {
        // determain whether is leaf node or not 
        if (len_key == 0) {
    ...
    // it is a middle node or the last childnode, return value will be examine in find() function
    return node;
}

template <typename K, typename T, typename Compare>
radix_tree_node<K, T, Compare>* radix_tree<K, T, Compare>::altree_find_node(const K &key, radix_tree_node<K, T, Compare> *node, int depth, bool compare, radix_tree_node<K, T, Compare> *prev_node)
{
    if (node == nullptr) {
        return nullptr;
    }
    // 1. find to leaf node(end).
    if (node->m_children.empty()) 
        return node;

    // an iterator to std::map.
    typename radix_tree_node<K, T, Compare>::it_child it;
    typename radix_tree_node<K, T, Compare>::it_reverse_child it_reverse;

    // 2. check if need to compare.
    if (compare) {
        // find the largest key node that less than or equal to Key.
    
        // m_children(std::map<key,node>), it(std::map<key,node>::iterator)
        // 3. start check child node.      
        for (it = node->m_children.begin(); it != node->m_children.end(); ++it) {
            // 3.1. check the first byte. 
            if (it->first[0] < key[depth]) {
                // 3.1.1. if the node key is small than the search key, record then next.
                prev_node = it->second;
                // prev_len = radix_length(it->first);
                continue;
            } else if (it->first[0] == key[depth]) {
                // 3.1.2. if the node key is equal to the search key, 
                int len_node = radix_length(it->first);
                K   key_sub  = radix_substr(key, depth, len_node); 
                if (it->first < key_sub) {
                    // 3.1.2.1 the first byte is equal, the node key small than key, it is the largest key that small than the search key.
                    return altree_find_node(key, it->second, depth+len_node, false, prev_node);
                } else if (it->first == key_sub){
                    // 3.1.2.2 keep search.
                    return altree_find_node(key, it->second, depth+len_node, true, prev_node);
                } else {
                    // 3.1.2.3 back to the prev node.
                    return altree_find_node(key, prev_node, depth+radix_length(prev_node->m_key), false, nullptr);
                }
            } else {
                // 3.1.3. back to the prev node. notice now is not the same depth.
                return altree_find_node(key, prev_node, prev_node->m_depth+radix_length(prev_node->m_key), false, nullptr);
            }
        }
    } else {
        // find the largest child directly.
        it_reverse = node->m_children.rbegin();
        int len_node = radix_length(it_reverse->first);
        K   key_sub  = radix_substr(key, depth, len_node);
        return altree_find_node(key, it_reverse->second, depth+len_node, false, nullptr);
    }
    return node;
}
